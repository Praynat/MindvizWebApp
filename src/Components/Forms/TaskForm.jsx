...
export default function TaskForm({
    ...
    const handleParentSelect = (item) => {
        setData(p => ({ ...p, parentIds: [item._id] }));
    };
    ...
    const handleWeekDayChange = (e) => {
        ...
        setData(p => {
            ...
            return {
                ...p,
                weekDays: checked ? [...days, name] : days.filter(d => d !== name)
            };
        });
    };
    const handleDateField = (field) => (val) =>
        setData(p => ({ ...p, [field]: val }));
    const handleAuto = (field) => (_, val) =>
        setData(p => ({ ...p, [field]: val }));
    ...
    return (
        <Form onSubmit={onSubmit} onReset={onReset} className="task-form">
            <LocalizationProvider dateAdapter={AdapterDateFns}>
                <Grid container spacing={3}>
                    {/* First line: Task Name, Deadline, Description */}
                    ...
                    {/* Second line: Parent Task, Task Type, Links, Tags */}
                    <Grid container item xs={12}>
                        <Grid item xs={12} sm={3}>
                            <Box sx={{ position: 'relative', display: 'inline-block', width: '100%', height: '100%' }}>
                                <Typography
                                    component="label"
                                    htmlFor="parent-select-button"
                                    sx={{
                                        position: 'absolute',
                                        top: -8,
                                        left: "8%",
                                ...
                                >
                                    <Box sx={{ position: 'relative', pb: 7 }}>
                                        <Box sx={{ maxHeight: 400, overflowY: 'auto', minWidth: 250 }}>
                                            {taskTree.map(item => (
                                                <SidebarItem
                                                    key={item._id}
                                                    item={item}
                                                    selectedItemId={selectedParentId}
                                                    onFilterSelect={handleParentSelect}
                                                />
                                            ))}
                                        </Box>
                                        <Button
                        ...
                        <Grid item xs={12} sm={3}>
                            <Autocomplete
                                ...
                                options={[]}
                                value={data.links || []}
                                onChange={handleAuto('links')}
                                renderTags={(val, getTag) =>
                                    val.map((opt, i) => <Chip key={i} label={opt} {...getTag({ i })} />)
                                }
                                renderInput={params => (
                                    <TextField
                                        {...params}
                    ...
                    {/* Third line: recurring Detail Fields */}

                    <Grid container item xs={12}>
                        {/* Conditional additional fields */}
                        {data.isFrequency && data.frequency === "Weekly" && (
                            <Grid item xs={12}>
                                <Box className="recurring-section">
                                    <FormControl component="fieldset" fullWidth error={!!errors.weekDays}>
                                        ...
                                        <Box display="flex" flexWrap="wrap" gap={0.5}>
                                            {weekDaysValues.map(d => (
                                                <FormControlLabel
                                                    key={d}
                                                    control={
                                                        <Checkbox
                                                            size="small"
                                                            checked={data.weekDays?.includes(d)}
                        ...
                        {data.isFrequency && data.frequency === "Monthly" && (
                            <Grid item xs={12} sm={4}>
                                <Input
                                    name="dayOfMonth"
                                    label="Day of Month"
                        ...
                        {data.isFrequency && data.frequency === "Yearly" && (
                            <Grid item xs={12} sm={4}>
                                <Input
                                    name="monthOfYear"
                                    label="Month"
                                    select
                                    options={monthOfYearValues.map(m => m.value)}
                                    getOptionLabel={opt => monthOfYearValues.find(m => m.value === opt)?.label || ''}
                                    data={data}
                                    onChange={onInputChange}
                        ...
                        {data.isFrequency && data.frequency === "Custom" && (
                            <Grid item xs={12} sm={4}>
                                <Input
                            ...
                            <Grid container spacing={2} alignItems="center">
                                <Grid item>
                                    <FormControlLabel
                                        control={
                                            <Checkbox
                                                checked={data.isFrequency || false}
                                                onChange={handleIsFreq}
                                                name="isFrequency"
                                            />
                                        }
                                    ...
                                    <>
                                        ...
                                        <Grid item xs={12} sm={3}>
                                            <DateTimePicker
                                                ...
                                                onChange={handleDateField('startDate')}
                                        ...
                                        <Grid item xs={12} sm={3}>
                                            <DateTimePicker
                                                ...
                                                label="End Date"
                                                value={data.endDate}
                                                onChange={handleDateField('endDate')}
